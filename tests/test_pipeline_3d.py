...
class Pipeline3DTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for path in (fo, dtl):
                writer = cv2.VideoWriter(
                    str(path), cv2.VideoWriter_fourcc(*"mp4v"), 25, (64, 64)
                )
            ...
            with (
                patch("pipeline_3d.create_session", side_effect=["det", "pose"]),
                patch(
                    "pipeline_3d.run_video",
                    side_effect=[[prediction(0)], [prediction(0)]],
                ) as infer,
            ):
    ...
    def test_smoothing_preserves_raw_data_and_drives_saved_viewer_and_projection(self):
        ...
        for disabled in (False, True):
            with (
                self.subTest(no_smooth=disabled),
                tempfile.TemporaryDirectory() as directory,
            ):
                root = Path(directory)
                video = root / "source.mp4"
                writer = cv2.VideoWriter(
                    str(video), cv2.VideoWriter_fourcc(*"mp4v"), 60, (64, 64)
                )
                for _ in frames:
                    writer.write(np.zeros((64, 64, 3), dtype=np.uint8))
                writer.release()
                ...
                with (
                    patch("pipeline_3d.create_session"),
                ...
                self.assertAlmostEqual(
                    float(actual[6, 10, 0]), float(expected_center), places=5
                )
                if disabled:
                ...
                np.testing.assert_allclose(
                    payload["comparison"]["fo"]["projected"],
                    actual[:, :, :2] + origin,
                    atol=1e-5,
                )
                np.testing.assert_allclose(
                    payload["comparison"]["dtl"]["projected"],
                    actual[:, :, [2, 1]] + origin,
                    atol=1e-5,
                )
                for name in ("fo", "dtl"):
                    self.assertEqual(
                        json.loads(paths[f"{name}_json"].read_text()), frames
                    )

    def test_missing_input_fails_before_loading_models(self):
        args = argparse.Namespace(
            fo_video=Path("missing-fo.mp4"),
            dtl_video=Path("missing-dtl.mp4"),
            output_dir=Path("out"),
            det_model=Path("det"),
            pose_model=Path("pose"),
            config=Path("config"),
            det_thr=0.4,
            kpt_thr=0.3,
            per_frame_origin=False,
